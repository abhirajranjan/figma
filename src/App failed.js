...
const App = () => {
  ...
  const buildHierarchy = (rectangles) => {
    const hierarchy = {
      id: "root",
    ...
    const addToHierarchy = (rect, parent) => {
      console.log(rect, parent);
      for (let child of parent.children) {
        if (isRectangleInside(rect, child)) {
          addToHierarchy(rect, child);
          return;
        }
      }
      parent.children.push({ ...rect, children: [] });
      console.log(rect, parent);
    };

    for (let rect of rectangles) {
      addToHierarchy(rect, hierarchy);
    }
  ...
  const exportToJson = () => {
    const hierarchy = buildHierarchy(rectangles);
    ...
    return JSON.stringify(flattenHierarchy(hierarchy), null, 2);
  };
